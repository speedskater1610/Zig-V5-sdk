...
/// Arcade drive control - left stick controls forward/backwards velocity, right stick controls rotation
fn arcadeDrive() void {
    // Get controller values
    const forward = vex.controller.vexControllerGet(CONTROLLER_ID, .Axis3);
    const turn = vex.controller.vexControllerGet(CONTROLLER_ID, .Axis1);

    // Apply deadband
    const fwd = applyDeadband(forward);
    const trn = applyDeadband(turn);

    // Calculate motor powers
    const left_power = fwd + trn;
    const right_power = fwd - trn;

    // Clamp to valid range (-100 to 100)
    const left_clamped = std.math.clamp(left_power, -100, 100);
    const right_clamped = std.math.clamp(right_power, -100, 100);

    // Set velocities
    vex.motor.vexDeviceMotorVelocitySet(left_motor, left_clamped);
    vex.motor.vexDeviceMotorVelocitySet(right_motor, right_clamped);
}
